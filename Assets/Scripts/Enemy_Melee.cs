...
public class Enemy_Melee : MonoBehaviour {
	...
	private bool playerJumpOver = false;
	...
	void Update () {
		...
		case WALK:
			...
			break;
		case ATTACK:
			//Debug.Log("transform..x "+transform.position.x+"player..x "+player.position.x+"moveLeft "+moveLeft);
			if(transform.position.x > player.position.x && !moveLeft ) // left
			{
				
				playerJumpOver = true;
			}else if(transform.position.x < player.position.x && moveLeft )
			{
				playerJumpOver = true;
			}
